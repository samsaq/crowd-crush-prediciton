...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        "Prepare image and density datasets", add_help=False
    )
    ...
    parser.add_argument(
        "--data_dir",
        default="primary_datasets/",
        type=str,
        help="Path to the original dataset",
    )
    parser.add_argument(
        "--mode", default="train", type=str, help="Indicate train or test folders"
    )

    # Output path
    parser.add_argument(
        "--output_dir",
        default="datasets/intermediate",
        type=str,
    ...
    parser.add_argument(
        "--kernel_size", default="", type=str, help="Size of the Gaussian kernel"
    )
    parser.add_argument(
        "--sigma", default="", type=str, help="Variance of the Gaussian kernel"
    )

    # Crop image parameters
    parser.add_argument(
        "--image_size", default=256, type=int, help="Size of the crop images"
    )
...
def main(args):
    ...
    img_list = sorted(
        glob.glob(os.path.join(data_dir, mode + "_data", "images", "*.jpg"))
    )
    ...
    kernel_list = []
    kernel_list = [
        create_density_kernel(kernel_size_list[index], sigma_list[index])
    ...
    for device, img_list in enumerate(sub_list):
        for file in img_list:
            ...
            file = (
                file.replace("images", "ground-truth")
                .replace("IMG", "GT_IMG")
                .replace("jpg", "mat")
            )
            ...
            if image_size == -1:
                ...
                if mode == "train" or mode == "test":
                    ...
                    images, densities = create_non_overlapping_crops(
                        image, density, image_size
                    )

            index = os.path.basename(file).split(".")[0].split("_")[-1]

            path = os.path.join(output_dir, f"part_{device+1}", mode)
            den_path = path.replace(
                os.path.basename(path), os.path.basename(path) + "_den"
            )
            ...
            for sub_index, (image, density) in enumerate(zip(images, densities)):
                file = os.path.join(
                    path, str(index) + "-" + str(sub_index + 1) + ".jpg"
                )

                if args.with_density:
                    req_image = [
                        (density[:, :, index] / normalizer[index] * 255.0)
                        .clip(0, 255)
                        .astype(np.uint8)
                        for index in range(len(normalizer))
                    ]
                ...
                image = (
                    np.concatenate(np.split(image, 2, axis=1), axis=0)
                    if args.with_density
                    else image
                )
                Image.fromarray(image, mode="RGB").save(file)
                density = (
                    rearrange(torch.tensor(density), "h w c -> h (c w)")
                    .detach()
                    .numpy()
                )
                file = os.path.join(
                    den_path, str(index) + "-" + str(sub_index + 1) + ".csv"
                )
...
def get_circle_count(image, normalizer=1, threshold=0, draw=False):
    ...
    th, threshedImg = cv2.threshold(
        denoisedImg, threshold, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU
...
def create_dot_map(locations, image_size):
    density = np.zeros(image_size[:-1])  # Remove channel dimension
    for x, y in locations:
        x, y = int(min(x, image_size[1] - 1)), int(
            min(y, image_size[0] - 1)
...
def resize_rescale_info(image, locations, image_size):
    ...
    # Scale locations according to the new dimensions
    locations = locations.copy()  # Create copy to avoid modifying original
    locations[:, 0] = np.clip(
        locations[:, 0] * w_scale, 0, image_size - 1
    )  # x coordinates
    locations[:, 1] = np.clip(
        locations[:, 1] * h_scale, 0, image_size - 1
    )  # y coordinates
...
def create_non_overlapping_crops(image, density, image_size):
    ...
    pad_density = rearrange(
        pad_density, "(p1 h) (p2 w) -> (p1 p2) h w", h=image_size, w=image_size
    ).numpy()
    pad_image = rearrange(
        pad_image, "(p1 h) (p2 w) c -> (p1 p2) h w c", h=image_size, w=image_size
    ).numpy()
...
def create_overlapping_crops(image, crop_size, overlap):
    ...
    image = arrange_crops(
        image=image, x_start=X_points, y_start=Y_points, crop_size=crop_size
    )
...
class GaussianKernel(nn.Module):
    ...
    def __init__(self, kernel_weights, device):
        ...
        self.kernel = nn.Conv2d(
            1, 1, kernel_weights.shape, bias=False, padding=kernel_weights.shape[0] // 2
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Prepare image and density dataset", parents=[get_arg_parser()]
    )
