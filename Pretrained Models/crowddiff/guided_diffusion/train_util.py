...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        # Initialize EMA parameters with the same shape as master parameters
        self.ema_params = []
        ...
        # Setup DDP if CUDA is available
        ...
        while (
            ...
            if self.step % 2000 == 0:
                ...
                while count < 0:
                    ...
                    model_kwargs = {
                        k: v.to(dist_util.dev()) for k, v in model_kwargs.items()
                    }
                    ...
                    sample = self.diffusion.p_sample_loop(
                        ...
                        (
                            1,
                            self.pred_channels,
                            model_kwargs["low_res"].shape[-2],
                            model_kwargs["low_res"].shape[-1],
                        ),
                    ...
                    crowd_den = [
                        (item + 1) * 0.5 * normalizer
                        for item, normalizer in zip(crowd_den, self.normalizer)
                    ]
                    crowd_den = [
                        item * 255 / (th.max(item) + 1e-12) for item in crowd_den
                    ]
                    ...
                    req_image = np.concatenate(
                        [req_image[0], crowd_img, req_image[-1]], axis=1
                    )
                    ...
                    path = os.path.join(
                        self.log_dir, f"results_{self.step}/{str(count)}.png"
                    )
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            # Update best values when we get a lower loss
            current_loss = (losses["loss"] * weights).mean().item()
            current_mse = losses["mse"].mean().item()
            current_mae = losses["mae"].mean().item()
    ...
    def log_step(self):
        ...
        # Log best values (corresponding to lowest loss)
        logger.logkv("best_loss", self.best_loss)
        logger.logkv("best_mse", self.best_mse)
        logger.logkv("best_mae", self.best_mae)
